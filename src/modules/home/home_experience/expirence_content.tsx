          ...
          <div className="mx-2 flex flex-col gap-y-4 rounded-xl bg-secondary py-5 px-3 dark:bg-secondary-dark">
            ...
            <span className="my-2 flex flex-wrap gap-2">
              {r.tech_stack.data?.map((t) => (
                <p
                  key={`tech_${t.id}`}
                  className="rounded-md bg-solid p-1.5 capitalize tracking-[0.2em] text-white dark:bg-solid-dark max-md:text-sm max-md:tracking-[0.1em]"
                >
...
export function HomeExperienceContent({ exp }: IProps) {
  ...
  return (
    <section className="flex w-full flex-col gap-y-9 rounded-[16px] bg-accent px-[30px] pt-9 pb-12 dark:bg-accent-dark max-md:gap-y-5 max-md:p-6">
      <div className="flex w-full flex-col">
        <span className="flex w-full items-center gap-y-6 py-1 max-md:flex-col-reverse max-md:items-start">
          ...
          <p className="rounded bg-solid/60 px-4 py-2 text-xs tracking-[0.05em] text-white dark:bg-secondary-dark max-md:self-end">
