...
export function HomeListing(p: IProps) {
  return (
    <HomeSection
      parentClass="bg-white dark:bg-black sticky -top-24 max-md:top-0"
      className="flex gap-y-2.5 px-9 py-[120px] max-md:px-2 max-md:py-9"
    >
      <AppTitle title={p.title} number={p.number} />
      <p className="mb-24 flex-1 whitespace-pre-line leading-9 tracking-[0.05em] text-black dark:text-white max-lg:text-lg max-md:mb-10 max-md:px-3 max-md:text-justify max-md:text-sm  max-md:leading-6">
        {p.description}
      </p>
      <div className="flex gap-8 max-md:hidden max-md:gap-2">
        {p.data.map((l) => (
          <ListingCard
            key={l.title}
            Icon={l.Icon}
