  ...
  if (!close)
    return (
      <section className="notification-backdrop  absolute inset-0 z-50 flex items-center justify-center bg-[#E6E8E7] dark:bg-[#04140C]">
        <section className="notification-popup flex w-full max-w-7xl  flex-col px-8 pt-6 max-sm:px-3">
          <div className="flex w-full items-center border-b border-b-primary pb-2 text-xl">
            <IC_MEGAPHONE className="inherit fill-primary dark:fill-primary-dark max-sm:hidden" />
            <h1 className="ml-4 flex-1 text-2xl font-bold capitalize tracking-widest text-primary dark:border-b-primary-dark  dark:text-white max-lg:text-lg max-sm:ml-0 max-sm:text-base">
              {t('Notification_title')}
            </h1>
            <button
              className="bg-primary px-6 py-2 text-base font-black tracking-wider text-white dark:bg-primary-dark max-sm:px-3"
              onClick={() => {
                setClose(true);
